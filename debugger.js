...
XMLRPCDebugger.Client = function(endpoint){
  this.endoint = endpoint;
  
  this.request = function(){
    args = Array.prototype.slice.call(arguments)
    callback = arguments.pop();
    method = arguments.unshift()
    params = arguments;
    
    console.log(methd, arguments, callback);
...
XMLRPCDebugger.Builder = function(methodName, methodParams){
  this.methodName = methodName;
  this.methodParams = methodParams;
    
}

XMLRPCDebugger.Builder.marshal = function(methodName, methodParams){
  return (new XMLRPCDebugger.Builder(methodName, methodParams)).toString();
}

XMLRPCDebugger.Builder.prototype.toString = function(){
  return "<?xml version=\"1.0\"?>\n" +
    "<methodCall>\n<methodName>" + this.methodName + "</methodName>\n" +
    "<params>\n" + this.buildParams(this.methodParams) + "</params>\n</methodCall>";
}

XMLRPCDebugger.Builder.prototype.buildParams = function(params){
  xml = "";
...
XMLRPCDebugger.Builder.prototype.encode = function(param){
  var x = XMLRPCDebugger.Builder;
  if (enyo.isArray(param)) {
    ...
    return "<dateTime.iso8601>" + XMLRPCDebugger.Builder.dateToIso8601(param) + "</dateTime.iso8601>";
...
XMLRPCDebugger.Builder.dateToIso8601 = function(date){
...
XMLRPCDebugger.Builder.isNumber = function(param){
  return (typeof(param) == 'number' || param instanceof Number);
}

XMLRPCDebugger.Builder.isDate = function(param) {
  if (param == null) return false;
  //console.log(">>> XMLRPCDebugger.Builder.isDate testing the paramter datatype: ", param,  typeof(param));  
...
XMLRPCDebugger.Builder.isString = function(param){
  return enyo.isString(param);
}

XMLRPCDebugger.Builder.isInt = function(param){
  return Math.round(param) == param;
}

XMLRPCDebugger.Builder.isObject = function(param){
  return (param instanceof Object || param.constructor == Object);
}

XMLRPCDebugger.Parser = function(string){
  ...
  var cleaned = XMLRPCDebugger.Parser.protectStrings(XMLRPCDebugger.Parser.cleanBOM(XMLRPCDebugger.Parser.cleanDocument(string)));
  this.document = doc = new DOMParser().parseFromString(cleaned, 'text/xml');
  //console.log("Parsed DOM", doc);
}

XMLRPCDebugger.Parser.parse = function(string){
  return (new XMLRPCDebugger.Parser(string)).toObject();
}

// prevents empty text nodes. and garbage between non-content XML-RPC nodes
XMLRPCDebugger.Parser.cleanDocument = function(string){
  var open, pairs = [
...
XMLRPCDebugger.Parser.cleanBOM = function(string){
...
XMLRPCDebugger.Parser.protectStrings = function(string){
...
XMLRPCDebugger.Parser.prototype.toObject = function(){
...
// TODO: Remove XPath and start using querySelector since we're dealing with a DOM object
XMLRPCDebugger.Parser.prototype.queryDocument = function(xpath, node){
  var results = [];
...
// TODO: Remove XPath and start using querySelector since we're dealing with a DOM object
XMLRPCDebugger.Parser.prototype.parse = function(node){
